# contoh seismic porepressure prediction 2D (supervised)
# - model dari geosc.ml.PorePressureRegressor.save(...)
# - urutan fitur wajib sama dengan training model
# - contoh urutan: [hydrostatic, overburden, AI, SI]
...
from geosc.seismic import SeismicPorePressurePredictor
...
# urutan atribut harus sama dengan FEATURE_COLS saat training well log porepressure
atb = [
...
output_segy = [f"/Drive/D/Temp/20260218_{line}_PP_PRED.sgy" for line in nmline]
...
predictor = SeismicPorePressurePredictor(
...
for out_path in output_segy:
    append_process_text_header(
        out_path,
        process_name="SeismicPorePressurePredictor",
        details=[
            f"model={model_path}",
            "feature_order=hydrostatic,overburden,...",
            f"n_attrs={len(atb)}",
            "null_value=-999.25",
        ],
    )
