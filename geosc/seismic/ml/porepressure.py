...
class SeismicPorePressurePredictor(SeismicMLBase):
    """
    Seismic pore pressure prediction within horizon interval (predict-only).
    ...
    - predict with trained ``geosc.ml.PorePressureRegressor``

    Important:
    - Input attribute order in ``input_segy_list`` must match training feature order.
    - Typically first features are [hydrostatic, overburden, ...].
    """
    ...
    def __init__(
        ...
        if self._n_attrs < 2:
            raise ValueError(
                "Pore pressure predictor requires at least 2 input attributes: "
                "[hydrostatic, overburden, ...]."
            )
        self.model_path = model_path

    @staticmethod
    def load_model(model_path: str) -> PorePressureRegressor:
        return PorePressureRegressor.load(model_path)
