...
def _require_sklearn():
    try:
        ...
        raise ImportError(
            "LithologyPredictor requires scikit-learn. Install with: pip install scikit-learn"
        ) from exc
...
class LithologyPredictor:
    """
    Train and run lithology classification models.
    ...
    def _transform(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.use_scaler:
            if self.scaler is None:
                raise ValueError("Scaler is enabled but not fitted. Call train() first.")
    ...
    @classmethod
    def load(cls, path: str) -> "LithologyPredictor":
