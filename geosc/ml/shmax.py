"""Shmax regression with target transform ratio to Shmin."""
...
class ShmaxRegressor(Regressor):
    """
    Specialized regressor for Shmax prediction.

    Required core inputs in X:
    - hydrostatic
    - overburden
    - porepressure
    - shmin
    ...
    def __init__(
        ...
        self.shmin_col = int(shmin_col)

    def _extract_core(self, X: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        shmin = X[:, self.shmin_col]
        return pp, hydro, ob, shmin
    ...
    def train(
        ...
        pp, hydro, ob, shmin = self._extract_core(X)
        ...
        if np.any(~valid_target):
            X = X[valid_target]
            y = y[valid_target]
            shmin = shmin[valid_target]
        if X.shape[0] == 0:
            raise ValueError("No valid samples after Shmax target filter.")

        tt = self._safe_divide(y, shmin)
        valid_tt = np.isfinite(tt)
        if np.any(~valid_tt):
            X = X[valid_tt]
            tt = tt[valid_tt]
        if X.shape[0] == 0:
            raise ValueError("No valid samples after Shmax target transform.")
    ...
    def predict(
        ...
        if null_value is not None:
            ...
            valid &= (target_pred != null) & (pp != null) & (hydro != null) & (ob != null) & (shmin != null)

        if null_value is None:
            out = np.full(X.shape[0], np.nan, dtype=float)
            out[valid] = shmax[valid]
            return out
    ...
    @classmethod
    def load(cls, path: str) -> "ShmaxRegressor":
        ...
        return cls(
            ...
            shmin_col=payload.get("shmin_col", 3),
